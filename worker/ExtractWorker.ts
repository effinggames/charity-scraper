...
import { IExtractPayload, IParsePayload } from 'shared/Types';
import { promisify } from 'typed-promisify';
import * as Xml2js from 'xml2js';

const parseString = promisify(Xml2js.parseString);

async function jobHandler(job: PgBoss.JobWithDoneCallback<IExtractPayload, any>) {
  ...
  const promises = xmlUrls
    .map(async (url) => {
      ...
      const payload: IParsePayload = { charityData: json };
